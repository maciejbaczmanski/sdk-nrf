...
#. Build the certification image.
   ...
   Use the :ref:`ot_cli_sample` sample as a base, apply the ``ci`` and ``multiprotocol`` snippets and set :kconfig:option:`SB_CONFIG_NETCORE_MULTIPROTOCOL_RPMSG` flag.
   ...
   * If building on the command line, use the following command:
     ...
     .. code-block::
        ...
        west build -b nrf52840dk/nrf52840 -- -Dcli_SNIPPET="ci;multiprotocol"  -DCONFIG_OPENTHREAD_LIBRARY=y -DSB_CONFIG_NETCORE_MULTIPROTOCOL_RPMSG=y
   ...
   * If building using Visual Studio Code, you must first `create and build the application <How to build an application_>`_ using the CLI sample.
     ...
     .. code-block::
        ...
        -DSB_CONFIG_NETCORE_MULTIPROTOCOL_RPMSG=y
